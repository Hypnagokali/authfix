...
use resvg::{tiny_skia, usvg};
    ...
    fn should_contain_valid_url() {
        ...
        pixmap.save_png("./out/qr_code.png").unwrap();

        let img: GrayImage = image::open("./out/qr_code.png").unwrap().to_luma8();
        let mut img = rqrr::PreparedImage::prepare(img);
        let grids = img.detect_grids();
        let (_, content) = grids[0].decode().unwrap();
        
        assert_eq!(content, format!("otpauth://totp/TestApp:john.doe@example.org?secret={}&issuer=TestApp&digits=6", secret));
