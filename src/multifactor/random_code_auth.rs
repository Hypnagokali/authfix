...
pub trait CodeSender {
    ...
    async fn send_code(&self, random_code: RandomCode) -> Result<(), Self::Error>;
}
    ...
    T: CodeSender + 'static,
{
    fn generate_code(
        ...
        Box::pin(async move {
            ...
            sender.send_code(random_code).await.map_err(|e| {
                cleanup_and_unknown_error(&session, "Could not send code to user", e)
            })?;
